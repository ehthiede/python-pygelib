...
def complex_kron_product(z1, z2, aggregate=False):
    ...
    b1, b2 = s1[:-3], s2[:-3]  # e.g. B x C x L
    s1, s2 = s1[-3:], s2[-3:]  # e.g. C x L x 2
    if not aggregate:
    ...
    s12 = b + (s1[0] * s2[0], s1[1]*s2[1], s1[2]*s2[2])

    s10 = b1 + (s1[0], 1) + torch.Size([s1[1], 1, s1[2], 1])
    s20 = b2 + (1, s1[0]) + torch.Size([1, s2[1], 1, s2[2]])
